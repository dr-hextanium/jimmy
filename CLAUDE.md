...
- `FeedforwardFit` + `PolePlacement` — SysId-style auto-tuning math for the turret's control law. `FeedforwardFit` recovers `(kV, kStatic)` from a steady-state power/velocity sweep and `kA = τ·kV` from a step-response time-constant fit (no acceleration differentiation); `PolePlacement` turns `(kV, kA)` into `(kP, kD)` for a chosen settling time. The on-robot `opmode/debug/TurretAutoTune.kt` (hold-to-run, motor-encoder-only, hard-limit-guarded) collects the samples and prints/pushes the five gains — it replaces hand-tuning the turret.
